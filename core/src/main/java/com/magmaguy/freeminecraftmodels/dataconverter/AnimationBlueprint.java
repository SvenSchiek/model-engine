...
import com.unityrealms.server.modelengine.model.animation.AnimationFrame;
import com.unityrealms.server.modelengine.model.animation.Keyframe;
...
import com.unityrealms.server.modelengine.transformation.TransformationType;
...
public class AnimationBlueprint {
    ...
    private void initializeIKAnimation(Map<String, Object> animationData, String controllerName, String modelName, String animationName) {
        ...
        for (Object keyframeData : (List) animationData.get("keyframes")) {
            ...
            if (keyframe.getTransformationType() == TransformationType.POSITION) {
    ...
    private void interpolateRotations(AnimationFrame[] animationFramesArray, List<Keyframe> rotationKeyframes) {
        ...
        for (int i = 0; i < rotationKeyframes.size(); i++) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                ...
                if (blockBenchVersion < 5) {
                    animationFramesArray[currentFrame].rotationX = interpolateWithType(interpType, previousFrame.getDataX(), animationFrame.getDataX(), t);
                    animationFramesArray[currentFrame].rotationY = interpolateWithType(interpType, previousFrame.getDataY(), animationFrame.getDataY(), t);
                    animationFramesArray[currentFrame].rotationZ = interpolateWithType(interpType, previousFrame.getDataZ(), animationFrame.getDataZ(), t);
                } else {
                    animationFramesArray[currentFrame].rotationX = -interpolateWithType(interpType, previousFrame.getDataX(), animationFrame.getDataX(), t);
                    animationFramesArray[currentFrame].rotationY = -interpolateWithType(interpType, previousFrame.getDataY(), animationFrame.getDataY(), t);
                    animationFramesArray[currentFrame].rotationZ = interpolateWithType(interpType, previousFrame.getDataZ(), animationFrame.getDataZ(), t);
        ...
        if (lastFrame != null && lastFrame.getTimeInTicks() < duration - 1) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                int currentFrame = j + previousFrame.getTimeInTicks();
                if (blockBenchVersion < 5) {
                    animationFramesArray[currentFrame].rotationX = lastFrame.getDataX();
                    animationFramesArray[currentFrame].rotationY = lastFrame.getDataY();
                    animationFramesArray[currentFrame].rotationZ = lastFrame.getDataZ();
                } else {
                    animationFramesArray[currentFrame].rotationX = -lastFrame.getDataX();
                    animationFramesArray[currentFrame].rotationY = -lastFrame.getDataY();
                    animationFramesArray[currentFrame].rotationZ = lastFrame.getDataZ();
        ...
        if (firstFrame != null && firstFrame.getTimeInTicks() > 0) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                if (blockBenchVersion < 5) {
                    animationFramesArray[j].rotationX = firstFrame.getDataX();
                    animationFramesArray[j].rotationY = firstFrame.getDataY();
                    animationFramesArray[j].rotationZ = firstFrame.getDataZ();
                } else {
                    animationFramesArray[j].rotationX = -firstFrame.getDataX();
                    animationFramesArray[j].rotationY = -firstFrame.getDataY();
                    animationFramesArray[j].rotationZ = firstFrame.getDataZ();
    ...
    private void interpolateTranslations(AnimationFrame[] animationFramesArray, List<Keyframe> positionKeyframes) {
        ...
        for (int i = 0; i < positionKeyframes.size(); i++) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                ...
                float t = j / (float) durationBetweenKeyframes;
                if (blockBenchVersion < 5) {
                    animationFramesArray[currentFrame].positionX = interpolateWithType(interpType, previousFrame.getDataX(), animationFrame.getDataX(), t) / 16f;
                    animationFramesArray[currentFrame].positionY = interpolateWithType(interpType, previousFrame.getDataY(), animationFrame.getDataY(), t) / 16f;
                    animationFramesArray[currentFrame].positionZ = interpolateWithType(interpType, previousFrame.getDataZ(), animationFrame.getDataZ(), t) / 16f;
                } else {
                    animationFramesArray[currentFrame].positionX = -interpolateWithType(interpType, previousFrame.getDataX(), animationFrame.getDataX(), t) / 16f;
                    animationFramesArray[currentFrame].positionY = interpolateWithType(interpType, previousFrame.getDataY(), animationFrame.getDataY(), t) / 16f;
                    animationFramesArray[currentFrame].positionZ = interpolateWithType(interpType, previousFrame.getDataZ(), animationFrame.getDataZ(), t) / 16f;
        ...
        if (lastFrame != null && lastFrame.getTimeInTicks() < duration - 1) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                int currentFrame = j + previousFrame.getTimeInTicks();
                if (blockBenchVersion < 5) {
                animationFramesArray[currentFrame].positionX = lastFrame.getDataX() / 16f;
                animationFramesArray[currentFrame].positionY = lastFrame.getDataY() / 16f;
                animationFramesArray[currentFrame].positionZ = lastFrame.getDataZ() / 16f;
                } else {
                    animationFramesArray[currentFrame].positionX = -lastFrame.getDataX() / 16f;
                    animationFramesArray[currentFrame].positionY = lastFrame.getDataY() / 16f;
                    animationFramesArray[currentFrame].positionZ = lastFrame.getDataZ() / 16f;
        ...
        if (firstFrame != null && firstFrame.getTimeInTicks() > 0) {
            ...
            for (int j = 0; j < durationBetweenKeyframes; j++) {
                ...
                animationFramesArray[j].positionX = firstFrame.getDataX() / 16f;
                animationFramesArray[j].positionY = firstFrame.getDataY() / 16f;
                    animationFramesArray[j].positionZ = firstFrame.getDataZ() / 16f;
                } else {
                    animationFramesArray[j].positionX = -firstFrame.getDataX() / 16f;
                    animationFramesArray[j].positionY = firstFrame.getDataY() / 16f;
                    animationFramesArray[j].positionZ = firstFrame.getDataZ() / 16f;
